...
def main():
    ...
    data = {}
    data['trainMeta/episode/rew'] = []
    data['trainMeta/step/rew'] = []
    data['testMeta/episode/rew'] = []
    data['testMeta/step/rew'] = []

    data_path = './' + recordFolder + '/data.pkl'
    ...
    while episodeCount < EPISODE_LIMIT and stepCount < STEPS_LIMIT:
        ...
        if episodeCount % 80 == 0:
            ...
            for _ in range(8):
                ...
                while not env.isTerminal() and episodeSteps <= maxStepsPerEpisode and goal_index < len(plantrace)-1 and not goal_not_found:
                    ...
                    # shortly we will have DQN training to rule those bad actions out.
                        ...
                        while not env.isTerminal() and not env.goalReached(goal) and episodeSteps <= maxStepsPerEpisode:
                            ...
                            action = agent_list[goal].selectMove(state)
                            externalRewards = env.act(actionMap[action])
                            test_episodeExternalRew += externalRewards
                            #stepCount += 1
                            episodeSteps += 1
                            nextState = env.getStackedState()
                ...
                test_rew.append(test_episodeExternalRew)
            
            print('Test rew : ', np.mean(test_rew))
            data['testMeta/episode/rew'].append([episodeCount, test_rew])
            data['testMeta/step/rew'].append([stepCount, test_rew])
            visualizer.add_entry(stepCount, "test rew/steps", np.mean(test_rew))
            visualizer.add_entry(episodeCount, "test rew/episode", np.mean(test_rew))

        
        episodeCumulativeRew.append(episodeExternalRew)
        visualizer.add_entry(stepCount, "train rew/steps", episodeExternalRew)
        visualizer.add_entry(episodeCount, "train rew/episode", episodeExternalRew)
        visualizer.add_entry(stepCount, "train average rew/steps", np.mean(episodeCumulativeRew))
        visualizer.add_entry(episodeCount, "train average rew/episode", np.mean(episodeCumulativeRew))

        data['trainMeta/episode/rew'].append([episodeCount, episodeExternalRew])
        data['trainMeta/step/rew'].append([stepCount, episodeExternalRew])


        data['R_table'] = R_table
        data['ro_table'] = ro_table
        data['ro_table_lp'] = ro_table_lp
